...
"""Betweeness keyphrase extraction model.
"""
...
class Betweeness(SingleRank):
    """Betweeness keyphrase extraction model.

    Parameterized example::
        import pke
        # 1. create a Betweeness extractor.
        extractor = pke.unsupervised.Betweeness()
    ...
    def candidate_weighting(self, window=10, weights=True):
        ...
        if weights:
            ...
            w = nx.betweenness_centrality(self.graph, weight='weight')
        else:
            # compute the word scores using random walk
            w = nx.betweenness_centrality(self.graph, weight=None)
