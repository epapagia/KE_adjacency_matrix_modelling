...
class SingleRank(TextRank):
        ...
        for sent_id, sentence in enumerate(self.sentences):
            for i, word in enumerate(sentence.stems):
                ...
                if word not in self.vocab.keys() and word not in stopwords_list and not str(word).isdigit() and len(str(word)) > 1:
                  ...
                  vocab_index += 1
                  self.word_to_sentID[word] = 1.0/(sent_id+1.0)
        return self.vocab
    ...
    def build_word_graph(self, window=10, pos=None):
        ...
        # add nodes to the graph
        self.graph.add_nodes_from([word for word, valid in text if valid and len(str(word)) > 1])
    ...
    def candidate_weighting(self, window=10, pos=None, normalized=False, weights=True, personalization=False):
        ...
        if personalization:
            ...
            w = nx.pagerank_scipy(self.graph,
                                  ...
                                  tol=0.0001,
                                  weight='weight',
                                  personalization=self.word_to_sentID)
        else:
            # compute the word scores using random walk
            w = nx.pagerank_scipy(self.graph,
                                  alpha=0.85,
        ...
        # loop through the candidates
        w_sorted = sorted(w.items(),key=operator.itemgetter(1),reverse=True)
