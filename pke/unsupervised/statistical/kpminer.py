...
class KPMiner(LoadFile):
    ...
    def candidate_selection(self, lasf=3, cutoff=400, stoplist=None, n=1, **kwargs):
    ...
    def candidate_weighting_wif(self, sigma=3.0, alpha=2.3, frequencies_common=None, frequencies_important=None, frequencies_common2=None, frequencies_important2=None):
        """Candidate weight calculation as described in the KP-Miner paper.
        ...
        for k, v in self.candidates.items():
            ...
            # get the df for unigram only
            if len(v.lexical_form) == 1:
                if k in frequencies_common.keys():
                    candidate_cf = frequencies_common.get(k, 0)
                if k in frequencies_important.keys():
                    candidate_if = frequencies_important.get(k, 0)
                if frequencies_common2 != None and frequencies_important2!=None:
                    if k in frequencies_common2.keys():
                      candidate_cf += frequencies_common2.get(k, 0)
                    if k in frequencies_important2.keys():
                      candidate_if += frequencies_important2.get(k, 0)

            # compute the wif score
            wif = math.sqrt((candidate_if)/(candidate_cf+1.0))

            if len(v.lexical_form) == 1:
                # If single word candidate do not apply boosting factor
                self.weights[k] = len(v.surface_forms) * wif
            else:
                 self.weights[k] = len(v.surface_forms) * B * wif
