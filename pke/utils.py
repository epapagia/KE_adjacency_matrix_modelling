...
def load_wif_file(input_file_common=None, input_file_important=None,
                                 delimiter='\t'):
    """Load a tsv (tab-separated-valueload_document_relative_var_files) file containing document frequencies.
    ...
    # open the input file
    with gzip.open(input_file_common, 'rt') if input_file_common.endswith('.gz') else \
            codecs.open(input_file_common, 'rt') as f:
        # read the csv file
        df_reader = csv.reader(f, delimiter=delimiter)

        # populate the dictionary
        for row in df_reader:
            frequencies_common[row[0]] = int(row[1])


     # initialize the DF dictionary
    frequencies_important = {}

    # open the input file
    with gzip.open(input_file_important, 'rt') if input_file_important.endswith('.gz') else \
            codecs.open(input_file_important, 'rt') as f:
        # read the csv file
        df_reader = csv.reader(f, delimiter=delimiter)

        # populate the dictionary
        for row in df_reader:
            frequencies_important[row[0]] = int(row[1])

    # return the populated dictionary
    return frequencies_common, frequencies_important

def compute_word_importance_factor(input_dir,
                            doci_file, doco_file, 
                            extension_common='_common_terms',
                            extension_important='_important_terms',
                            delimiter='\t'):
    """Compute the relative variance scores from a set of input documents. An
    ...
    dict_points_scores_common = {}

    # initialize number of documents
    nb_documents = 0

    # loop through the documents
    for input_file in glob.iglob(input_dir + '/*' + extension_common):
      with open(input_file, 'rb') as handle:
        dict_points_scores_local = pickle.load(handle)
      print('length dict_points_scores_local:', len(dict_points_scores_local.keys()), 'length dict_points_scores_common:', len(dict_points_scores_common.keys()))
      r = {x: dict_points_scores_local.get(x, 0) + dict_points_scores_common.get(x, 0) for x in set(dict_points_scores_local).union(dict_points_scores_common)}
      
      dict_points_scores_common = r
      nb_documents += 1
    ...
    if os.path.dirname(doci_file):
        os.makedirs(os.path.dirname(doci_file), exist_ok=True)

    # dump the df container
    with gzip.open(doci_file, 'wb') as f:
        ...
        for ngram in dict_points_scores_common:
            line = ngram + delimiter + str(dict_points_scores_common[ngram])
            f.write(line.encode('utf-8') + b'\n')



    # document frequency container
    dict_points_scores_important = {}

    # initialize number of documents
    nb_documents = 0

    # loop through the documents
    for input_file in glob.iglob(input_dir + '/*' + extension_important):
      with open(input_file, 'rb') as handle:
        dict_points_scores_local = pickle.load(handle)
      print('length dict_points_scores_local:', len(dict_points_scores_local.keys()), 'length dict_points_scores_important:', len(dict_points_scores_important.keys()))
      r = {x: dict_points_scores_local.get(x, 0) + dict_points_scores_important.get(x, 0) for x in set(dict_points_scores_local).union(dict_points_scores_important)}
      
      dict_points_scores_important = r
      nb_documents += 1
    ...
    # create directories from path if not exists
    if os.path.dirname(doco_file):
        os.makedirs(os.path.dirname(doco_file), exist_ok=True)

    # dump the df container
    with gzip.open(doco_file, 'wb') as f:

        # add the number of documents as special token
        first_line = '--NB_DOC--' + delimiter + str(nb_documents)
        f.write(first_line.encode('utf-8') + b'\n')

        for ngram in dict_points_scores_important:
            line = ngram + delimiter + str(dict_points_scores_important[ngram])
            f.write(line.encode('utf-8') + b'\n')
