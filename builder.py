...
from gowpy.gowpy.gow.typing import Token, Tokenized_document, Tokenizer, Edge, Edge_with_code, Edge_label, Edges, Nodes
import networkx as nx
from defaults import default_tokenizer
...
class GoWBuilder(object):
    ...
    def compute_gow_from_tokenized_document(self, vocab_kcore, tokens: Tokenized_document) -> GraphOfWords:
        nodes = set()
        token_ids = []
        for token in tokens:
            token_id = self.get_code_(token)
            token_ids.append(token_id)
            if token in vocab_kcore.keys():
              nodes.add(token_id)
        ...
        if self.edge_labeling:
            if self.directed:
                ...
                for j in range(N):
                    for i in range(max(j - self.window_size + 1, 0), j):
                    # for i in range(j + 1, min(j + self.window_size, N)):
                        # Only keep edges between two *different* tokens
                        if token_ids[i] != token_ids[j] and token_ids[i] in nodes and token_ids[j] in nodes:
                            edge = (token_ids[i], token_ids[j])
                            edge_code = self.get_edge_code_(edge)
    ...
    def compute_gow_from_document(self, vocab_kcore, raw_document: str) -> GraphOfWords:
        """Computes a graph-of-words representation from a document"""
        tokens = self.tokenizer(raw_document)
        return self.compute_gow_from_tokenized_document(vocab_kcore, tokens)
