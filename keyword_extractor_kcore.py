...
from builder import GoWBuilder
from gowpy.gowpy.gow.typing import Tokenizer

from networkx.algorithms.core import k_shell, k_core, core_number as nx_core_number
from networkx.classes.function import density

from algorithms import core_number_weighted

import warnings

from helpers import elbow
...
class KcoreKeywordExtractor(object):
    ...
    def extract(self, vocab_kcore, document: str) -> Sequence[Tuple[str, float]]:
        """Extracts keywords from the given document"""
        if self.selection_method == GOW_KEYWORD_EXTRACTION_KCORE:
            return self.extract_k_core(vocab_kcore, document)
    ...
    def extract_k_core(self, vocab_kcore, document: str) -> Sequence[Tuple[str, float]]:
        """Extraction of keywords corresponding to the nodes of the main core"""
        # Building the graph-of-words
        gow = self.builder.compute_gow_from_document(vocab_kcore, document)
        graph = gow.to_graph()
